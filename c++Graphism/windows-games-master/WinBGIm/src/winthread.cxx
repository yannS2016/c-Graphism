...
static long tempLeft = 0;
static long tempTop = 0;
static long resetLeft = 0;
static long resetTop = 0;
...
DWORD WINAPI BGI__ThreadInitWindow( LPVOID pThreadData )
{
    ...
    hWindow = CreateWindowEx( 0,                    // Extended window styles
			      ...
			      pWndData->title.size( )
			      ? (WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_SIZEBOX | WS_DLGFRAME)
    ...
    //SetWindowLong( hWindow, GWL_STYLE, WS_SIZEBOX);
...
static void cls_OnPaint( HWND hWnd )
{
    ...
    /**********************************************************
    // When window startup, ps.rcPaint.left is equal to 0
    // and ps.rcPaint.right becomes the user secified width - 10
    // for each resize event a new painting area define by rcPaint must be determined
    // therefore, a new srcCorner.x(origin x new rect), srcCorner.y(origin y of new rect)
    // new width must be based on old witdh as well as new height based on old height
    // in order word we need to track down the top left coordinates and width and height of
    // rcPaint  drawing area(area that need updating).
    ********************************************************************************************/
// when windows get resized:
    tempLeft = ps.rcPaint.left;
    if(ps.rcPaint.left == (ps.rcPaint.right - 1)){
        tempTop = 0;
        resetLeft = ps.rcPaint.left;
        std::cout << "resize horizontal: Top was reset\n";
    }
    if(ps.rcPaint.top == (ps.rcPaint.bottom - 1)){
        if(tempTop == 0)
        {
            tempTop = 290;
            tempLeft = resetLeft;
            ...
            tempTop = ps.rcPaint.top;
            tempLeft = resetLeft;
        }
        std::cout << "resize vertical: Left was reset\n";
    }
    ...
    //std::cout << "top is is:  " << ps.rcPaint.top << " Left is: " << ps.rcPaint.left << std::endl;
   // std::cout << "Origin top is:  "  << srcCorner.y  <<std::endl;
    ...
    std::cout << "top is:  " << ps.rcPaint.top << " Left is: " << ps.rcPaint.left
              << " right is:"<< ps.rcPaint.right <<std::endl;
    // MGM: Screen BitBlts are not always successful, although I don't know why.
    success = BitBlt( ps.hdc, tempLeft, tempTop, width, height,
			 hSrcDC, 0, 0, SRCCOPY );
